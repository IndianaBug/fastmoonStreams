...
class CommonFlowFunctionalities():
    ...
    def reference_stream_data(self, stream_data):
       ...
       self.id = stream_data.get("id_ws") if stream_data.get("id_ws") else stream_data.get("id_api")

    def reference_saving_directory(self, folderpath):
       """ passes connectiondata dictionary"""
       self.folderpath = folderpath
       
    def reference_logger(self, logger):
       """ passes connectiondata dictionary"""
       self.logger = logger
    ...
    def merge_ticks_buys_sells(uptick_data, down_tick):
        ...
        down_tick = {
                key: [{"quantity": -entry["quantity"], "price": entry["price"]} for entry in value]
...
class depthflow(CommonFlowFunctionalities):
    ...
    async def schedule_snapshot(self):
        ...
        try:
            ...
            self.logger.error("Keep-Alive error, connection closed: %s, id: %s", e, self.id, exc_info=True)
...
class oiflow(CommonFlowFunctionalities):
    ...
    async def input_data(self, data, *args, **kwargs):
        ...
        try:
            ...
            # if self.stream_data.get("id_api") == "kucoin_api_perpetual_oifunding_xbtusdtm":
            #     print(oidata, self.ois.data)
        except:
            return
    ...
    async def schedule_aggregation_oi_deltas(self, aggregation_lag:int=1, *args, **kwargs):
        ...
        try:
            ...
            while True:
                ...
                oi_ticks = list(self.market_state.raw_data.get("ticks_data_to_merge").get("oi_deltas").values())
                                
                merged_ticks = self.merge_ticks(oi_ticks)
        ...
        try:
            ...
            while True:
                ...
                if "cdepth" in aggregation_type:
                    ...
                    depth_dict = self.make_rbooks_dictionary([depth, trades])
                    self.market_state.staging_data["maps"][aggregation_type] = depth_dict
    ...
    async def schedule_aggregation_oioption(self, aggregation_lag=1, *args, **kwargs): 
        try:
            ...
            while True:
                ...
                self.market_state.staging_data["oi_options"] = oi_dictionary
