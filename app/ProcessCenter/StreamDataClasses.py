...
class MarketState:
    ...
    def __init__(self, streams_data, cdepth_spot=True, cdepth_future=True, rdepth_spot=True, rdepth_future=True, dead_instruments_timeout=600):
        ...
        self.calculator = GlobalMetricsCalculator()
        self.calculator.pass_datas(self.staging_data, self.raw_data)
        # self.calculator.calculate_global_metrics()
    ...
    def create_marketstate_datastructure(self, streams_data, cdepth_spot, cdepth_future, rdepth_spot, rdepth_future, *args, **kwargs,):
        ...
        if {("oi", "option")}.issubset(metrics_to_insttype) or {("oi", "oioption")}.issubset(metrics_to_insttype):
            global_data_structure["puts"] = 0
            global_data_structure["calls"] = 0
            oi_option = True
...
class GlobalMetricsCalculator:
    ...
    def pass_datas(self, staging_data, raw_data):
        self.staging_data = staging_data
        self.raw_data = raw_data

    def calculate_weighted_average(self, prices, weights):
    ...
    def extract_values(self, nested_dict):
        ...
        return [value for items in nested_dict.values() for value in items.values()]

    def calculate_global_price_spot(self):
        """Calculate the global spot price."""
        try:
            spot_prices = list(self.staging_data["by_instrument"]["price_spot"].values())
            self.staging_data["global"]["price_spot"] = np.average(spot_prices)
        except KeyError:
            pass  # Skip calculation if data is missing

    def calculate_global_price_future(self):
        """Calculate the global future price weighted by OI."""
        try:
            future_prices = list(self.staging_data["by_instrument"]["price_future"].values())
            future_ois = [self.staging_data["by_instrument"]["oi_future"].get(symbol) 
                          for symbol in self.staging_data["by_instrument"]["price_future"].keys()]
            self.staging_data["global"]["price_future"] = self.calculate_weighted_average(future_prices, future_ois)
        except KeyError:
            pass  # Skip calculation if data is missing

    def calculate_volumes_spot(self):
        """Calculate the global volumes."""
        try:
            self.staging_data["global"]["buys_spot"] = self.calculate_sum(list(self.staging_data["maps"]["buys_spot"].values()))
            self.staging_data["global"]["sells_spot"] = self.calculate_sum(list(self.staging_data["maps"]["sells_spot"].values()))
        except KeyError:
            pass  # Skip calculation if data is missing
        
    def calculate_volumes_future(self):
        """Calculate the global volumes."""
        try:
            self.staging_data["global"]["buys_future"] = self.calculate_sum(list(self.staging_data["maps"]["buys_future"].values()))
            self.staging_data["global"]["sells_future"] = self.calculate_sum(list(self.staging_data["maps"]["sells_future"].values()))
        except KeyError:
            pass  # Skip calculation if data is missing

    def calculate_oi_future(self):
        """Calculate the global OI future."""
        try:
            self.staging_data["global"]["oi_future"] = self.calculate_sum(list(self.staging_data["by_instrument"]["oi_future"].values()))
        except KeyError:
            pass  # Skip calculation if data is missing

    def calculate_liquidations(self):
        """Calculate the global liquidations."""
        try:
            self.staging_data["global"]["longs"] = self.calculate_sum(list(self.staging_data["maps"]["longs"].values()))
            self.staging_data["global"]["shorts"] = self.calculate_sum(list(self.staging_data["maps"]["shorts"].values()))
        except KeyError:
            pass  # Skip calculation if data is missing

    def calculate_oi_options(self):
        """Calculate the global OI options."""
        try:
            puts_values = self.extract_values(self.staging_data["oi_options"]["puts"])
            calls_values = self.extract_values(self.staging_data["oi_options"]["calls"])
            self.staging_data["global"]["puts"] = self.calculate_sum(puts_values)
            self.staging_data["global"]["calls"] = self.calculate_sum(calls_values)
        except KeyError:
            pass  # Skip calculation if data is missing

    def calculate_funding(self):
        """Calculate the global funding."""
        try:
            funding_data = {key: {"funding": value} for key, value in self.staging_data["by_instrument"]["funding"].items()}
            for symbol, data in funding_data.copy().items():
                oi = self.staging_data["by_instrument"]["oi_future"].get(symbol)
                if oi is not None:
                    data["oi"] = oi
                else:
                    del funding_data[symbol]
    
            fundings = [item["funding"] for item in funding_data.values()]
            ois = [item["oi"] for item in funding_data.values()]
            self.staging_data["global"]["funding"] = self.calculate_weighted_average(fundings, ois)
        except KeyError:
            pass  # Skip calculation if data is missing

    def calculate_global_metrics(self):
        """Calculate all global values."""
        self.calculate_global_price_spot()
        self.calculate_global_price_future()
        self.calculate_volumes_spot()
        self.calculate_volumes_future()
        self.calculate_oi_future()
        self.calculate_liquidations()
        self.calculate_oi_options()
        self.calculate_funding()
