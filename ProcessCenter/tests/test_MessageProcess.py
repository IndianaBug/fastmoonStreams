...
async def create_testing_object(streams_data, parent_dir):
    ...
    for stream_data in streams_data:
        ...
        #     # Simulate some state and stream data as needed
        #     try:
...
#     fail_threshold = message_processor_fail_threshold
#     market_state = MarketState(streams_data)
#     return streams_data, fail_threshold, market_state
...
# @pytest.mark.asyncio
# async def test_dynamic(test_params, setup_market_state):
#     streams_data, fail_threshold, market_state = setup_market_state
#     id_ = test_params["id"]
#     data_generator = test_params["data_generator"]
#     processing_method = test_params["processing_method"]
#     method_tester = test_params["method_tester"]

#     results = []
#     for data in data_generator():
#         rrr = await processing_method(data, market_state, market_state.streams_data.get(id_))
#         result = method_tester(rrr)
#         results.append(result)

#     true_count = results.count(True)
#     total_count = len(results)
#     assert true_count >= fail_threshold * total_count, f"Only {true_count}/{total_count} ({(true_count/total_count)*100:.2f}%) passed"
...
#         return [name for name, method in inspect.getmembers(self, predicate=inspect.isfunction)]

#     def open_json_file(self, path):
#         with open(path, 'r') as file:
#             return json.load(file)

#     def create_test_method(self, id_, data_generator, processing_method, method_tester):
#         """Factory to create a test method."""
#         async def test_method(self):
#             results = []
#             for data in data_generator():

#                 rrr = await processing_method(data, self.market_state, self.market_state.streams_data.get(id_))
#                 print(rrr)
#                 result = method_tester(rrr)
...
#             true_count = results.count(True)
#             total_count = len(results)
#             self.assertTrue(true_count >= self.fail_threshold * total_count, f"Only {true_count}/{total_count} ({(true_count/total_count)*100:.2f}%) passed")

#         return test_method

#     def add_method_to_class(self, id_, testing_method):
#         """
...
#         """
#         setattr(self, id_, testing_method)

#     def add_testing_methods(self, test_mapping):
#         """Creates a test case class dynamically from a mapping."""
        
#         test_mapping = self.create_testing_object()
        
#         for methods_stack in test_mapping:
            
#             id_ = methods_stack.get("id")
#             data_generator = methods_stack.get("data_generator")
#             processing_method = methods_stack.get("processing_method")
#             method_tester = methods_stack.get("method_tester")

#             testing_method = self.create_test_method(id_, data_generator, processing_method, method_tester)
#             self.add_method_to_class(id_, testing_method)
