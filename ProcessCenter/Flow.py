...
class booksflow():
    ...
    def __init__(self, 
        ...
        self.df = dd.from_pandas(pd.DataFrame(), npartitions=self.npartitions)
    ...
    def dfs_input_books(self):
        ...
        if self.df is None or self.df.npartitions == 0:
            self.df = dd.from_pandas(pd.DataFrame(0, index=list(range(60)), columns=columns, dtype='float64'), npartitions=self.npartitions)
            self.df = self.df.set_index(self.current_second)
    ...
    def get_books_snapshot(self):
        ...
        ax = pandas_series.plot()
        ax.set_xlabel('Level')
        ax.set_ylabel('Amount')
        ax.legend([f'Books for every level at timestmap {timestamp}'])
        plt.show()
